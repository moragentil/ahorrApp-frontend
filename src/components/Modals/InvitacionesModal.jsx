                ...
                <div key={inv.id} className="bg-muted/30 border border-border rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      ...
                      {/* Mostrar si se vinculará con un participante existente */}
                      {inv.participante_asociable && (
                        <div className="mt-3 p-2 bg-primary/10 border border-primary/20 rounded-lg">
                          <div className="flex items-center gap-2 text-sm">
                            <Link className="w-4 h-4 text-primary" />
                            <span className="text-foreground">
                              Se vinculará con <span className="font-medium">{inv.participante_asociable.nombre}</span>
                            </span>
                          </div>
                          {inv.participante_asociable.total_gastos > 0 && (
                            <p className="text-xs text-muted-foreground mt-1 ml-6">
                              Heredarás {inv.participante_asociable.total_gastos} gasto(s) asociado(s)
                            </p>
                          )}
                        </div>
                      )}
